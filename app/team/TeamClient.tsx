...
export default function TeamClient({ teamMembers }: { teamMembers: TeamMember[] }) {
    ...
    const onScroll = () => {
      ...
      if (foundersRef.current) {
        ...
        if (scrolled >= 0 && scrolled <= scrollDistance) {
          ...
          setActiveIndex(
            Math.min(teamMembers.length - 1, Math.max(0, Math.floor(progress * teamMembers.length)))
          );
        } else if (scrolled < 0) {
          setActiveIndex(0);
        } else {
          setActiveIndex(Math.max(0, teamMembers.length - 1));
        }
      }
    };
  ...
  const active = teamMembers.length > 0 ? teamMembers[activeIndex] : null;
  ...
  return (
    <>
      ...
      <style jsx>{`
        ...
        .tm-stage-body {
          display: grid;
          grid-template-columns: minmax(0, 1.1fr) minmax(220px, 420px);
